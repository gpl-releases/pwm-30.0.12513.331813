/*
 *
 * Copyright (C) 2010-2011 Bill Gatliff <bgat@billgatliff.com>
 *
...
int pwm_get_duty_percent(struct pwm_channel *p)
{
	if (0 == pwm_get_period_ns(p)) return -1;
	else
	return pwm_get_duty_ns(p)*100/pwm_get_period_ns(p);
}
...
int pwm_get_polarity(struct pwm_channel *p)
{

	return p->active_high;
}
EXPORT_SYMBOL(pwm_get_polarity);
...
void  __exit pwm_core_exit(void)
{
